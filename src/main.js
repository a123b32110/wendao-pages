...
setShared(effects,`p:${c.uid}`,next);
shared.set(`p:${c.uid}`,next);
}
}
...
pods.appendChild(h('div',{class:'pod '+x[1]},[h('span',{class:'sl',text:LBSEAL[r.rank]||String(r.rank)}),h('div',{class:'pn',title:r.n,text:r.n}),h('div',{class:'pv',text:lbVal(r)})]))});
...
var v=await rpc(sub==='ach'?'ach':sub==='life'?'bio':'bounty',{},{quiet:true});if(!v)return;var d=v.data;var m=S.me;
var nav=h('div',{class:'row'},[['bounty','悬赏'],['ach','成就'],['life','传记']].map(function(t){return h('button',{class:sub===t[0]?'pri sm':'sm',onclick:function(){bioLoad(t[0])}},t[1])}));
var kids=[hero('bio')];
...
out.push(h4(`${p.name} ${p.lv} 级${["", "（化形）", "（仙形）"][p.ev] ?? ""}`),muted(`${ELI[p.elem] ?? ""}${p.elem} · 攻 ${fmt(p.atk)} · 血 ${fmt(p.hp)} · 气血 ${Math.round((p.hpP ?? 1) * 100)}%`),P(p.xp,p.need));
...
const out=[sc_subnav(ui,[["bounty","悬赏"],["ach","成就"],["life","传记"]])];
if(sub==="bounty"){
const b=d.bounty;
