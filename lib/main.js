...
import { indexShared, byPrefix, setShared, delShared, setSharedSoft, sharedRoomFor, PX_BUCKETS, AX_BUCKETS, SX_BUCKETS, pxKey, axKey, sxKey } from "./game/shared.js";
...
import { explore, choose, regionsView, eventView, EXPLORE_DAILY } from "./game/explore.js";
import { inventoryView, useItem, equip, unequip, sell, sellArtifact, setGongfa, setArts } from "./game/inventory.js";
import { recipesView, craft } from "./game/craft.js";
import { shopView, buy, shopRefresh, refreshCost, SHOP_REFRESH_DAILY } from "./game/shop.js";
import { auctionsView, createAuction, bid, claimAuctions, botSettleAuctions } from "./game/auction.js";
import { candidates, refresh, fight, syncDefense, ARENA_DAILY, ATK_KEEP } from "./game/arena.js";
...
import { worldFor, attackWorldBoss, bossBoard, bossMine, claimBossReward, attackSectBoss, sectBossBoard, BOSS_DAILY } from "./game/boss.js";
...
// job_key 从 "tick" 换成 "tick2"：真实安装上旧键的待决任务在一场整批被拒的风暴后
// 再也不触发了（疑似平台侧的挂起标记没清），同名再加会被吞掉。换个名字绕开僵尸任务。
function ensureTick(shared, now, effects) {
  const w = shared.get("world");
  if (!w || now - (w.tickAt ?? 0) > 2 * TICK_SECONDS * 1000) {
    effects.push({ type: "schedule.add", job_key: "tick2", in_seconds: 20 });
  }
}
...
function botWork(shared, now, effects, budget, tickAt) {
  ...
  const foldBucket = (bk, pfx, keyOf, keep, age) => {
    if (room() < 4) return;
    // 顶到配额时**不要新建桶键**：一批 effects 是单事务，多出来的新键会把整批（连同删除）一起顶回来，
    // 于是永远腾不出地方——死锁。这一轮只让清扫去删，等有了空位下一轮再建桶。
...
const ART_VIEW = (c) => ({
  gongfa: GONGFA.filter((g) => c.gfs.includes(g.id)).map((g) => ({ ...g, equipped: c.gf === g.id, locked: !!(g.path && g.path !== c.path) || !!(g.demon && c.path !== "xie" && c.r >= 1) })),
  arts: ARTS.filter((a) => c.arts.includes(a.id)).map((a) => ({ ...a, equipped: c.eqArts.includes(a.id), locked: !!(a.path && a.path !== c.path) })),
  eqArts: c.eqArts,
});
...
function tabData(tab, c, shared, now, bio, legacy, sub) {
  switch (tab) {
    ...
    case "bio": return { bio: bio.slice().reverse().slice(0, 30), legacy: { pts: legacy.pts, lives: legacy.lives, history: legacy.history ?? [] }, stats: c.stats, bounty: bountyView(c), ach: achView(c, legacy) };
    default: return {};
  }
}
  ...
  try {
    ...
    switch (method) {
      ...
      case "bt": {
        ...
        if (res.ok && res.success) bio = pushBio(bio, `突破至${realmName(c.r, c.s)}`, now, "bt");
      ...
      // ---- explore
      case "regions": data = { regions: regionsView(c), event: eventView(c, st()), daily: EXPLORE_DAILY }; break;
      case "explore": {
      ...
      case "arena.fight": {
        ...
        data = { arena: { list: candidates(c, shared, now), left: ARENA_DAILY - c.daily.arena, refresh: 3 - (c.daily.arenaRefresh ?? 0), season: c.season }, battle: res.battle ?? null };
        break;
      }
      // ---- sect
      ...
      // ---- world boss
      case "boss": data = { boss: { world, board: bossBoard(shared, dayKey(now)), left: BOSS_DAILY - c.daily.boss, mine: bossMine(shared, dayKey(now), uid)?.d ?? 0 } }; break;
      case "boss.attack": res = attackWorldBoss(c, shared, now, effects); data = { boss: { world, board: bossBoard(shared, dayKey(now)), left: BOSS_DAILY - c.daily.boss, mine: (bossMine(shared, dayKey(now), uid)?.d ?? 0) + (res.dealt ?? 0) }, battle: res.battle ?? null }; break;
      // ---- boards / bio
...
export async function onSchedule(ctx, api) {
  ...
  effects.push({ type: "schedule.add", job_key: "tick2", in_seconds: TICK_SECONDS });
