...
#wd .tag.vipb{font-size:10px;padding:0 5px;background:rgba(0,0,0,.25)}#wd .name.vip4{text-shadow:0 0 8px rgba(255,154,60,.7)}
/* ---- reward reveal */
...
export function pageJs(assets) {
  return "(function(){var A=" + JSON.stringify(assets ?? {}) + ";var wxSim=" + WX_SIM_SRC + ";" + String.raw`
...
var KINDN={mat:'材料',pill:'丹药',tal:'符箓',egg:'兽卵',misc:'杂物',art:'法宝'};
...
// v48 活动横幅 + 师徒
var ev=d.event;if(ev&&(ev.list.length||ev.next))kids.push(h('div',{class:'card'},[h('div',{class:'row sb'},[h('span',{class:'row'},[h('b',{text:'今日活动'})].concat(ev.list.length?ev.list.map(function(e){return h('span',{class:'tag gold',text:e.name+' · '+e.desc})}):[h('span',{class:'muted',text:'今日无事'})])),ev.next?h('span',{class:'muted',text:'今晚 '+ev.next+'：掉落 ×1.5'}):null]),h('div',{class:'muted',text:'周一/周四双修日修炼 ×1.5 · 周三坊市集八折 · 周六日秘境 +1 次 · 每晚八至十点妖潮'})]));
...
var VIPC=['','#c9ced6','#e2b84a','#7fd3ff','#ff9a3c'];
function vipBadge(lv){lv=lv|0;if(!lv)return null;return h('span',{class:'tag vipb',style:'border-color:'+VIPC[lv]+';color:'+VIPC[lv],text:['','白银','黄金','钻石','王者'][lv]})}
function vipCard(v){if(!v)return null;var pct=v.need?Math.min(100,Math.round(v.en/v.need*100)):100;
return h('div',{style:'margin-top:12px;border-top:1px solid var(--line);padding-top:8px'},[h('div',{class:'row'},[h('b',{text:'会员 · '+v.name}),vipBadge(v.lv),h('span',{class:'muted',text:'累计供奉 '+v.en+' 点'+(v.next?'，再 '+(v.need-v.en)+' 点升 '+v.next:'，已至顶')})]),h('div',{class:'bar xp'},[h('i',{style:'width:'+pct+'%'})]),h('div',{class:'list',style:'margin-top:6px'},v.all.slice(1).map(function(t){return h('div',{class:'row'+(t.lv<=v.lv?'':' muted')},[h('span',{style:'min-width:3.5em;color:'+VIPC[t.lv],text:t.name+' '+t.en}),h('span',{class:'grow',text:t.perks.join(' · ')})])})),h('div',{class:'muted',text:'等级按累计供奉的能量升，跨转世永久有效。'})])}
...
var head=h('div',{class:'encp'},[portrait('mon_'+(p.mon||''),p.name,'mon'),h('div',{class:'grow'},[h('div',{class:'n row'},[tierTag(p.eggTier||0),p.name+'　'+p.lv+' 级'+['','　化形','　仙形'][p.ev||0]]),h('div',{class:'muted'},[elSpan(p.elem),'攻击 '+fmt(p.atk)+' · 气血 '+fmt(p.hp)]),h('div',{class:'bar hp'},[h('i',{style:'width:'+hpp+'%'})]),h('div',{class:'bar xp'},[h('i',{style:'width:'+(pv.maxed?100:pct(p.xp,p.need))+'%'})]),h('div',{class:'muted',text:'气血 '+hpp+'% · 历练 '+(pv.maxed?'圆满':p.xp+'/'+p.need)})])]);
var trip;
...
var vs=d.vshop;var vcard=null;if(vs){vcard=h('div',{class:'card'},[h('div',{class:'row'},[h('h3',{text:'珍宝阁'}),vipBadge(vs.lv)]),h('div',{class:'sub',text:vs.lv?'会员专属货架，每日更新，贵一些但保证有货':'白银会员（累计供奉 '+vs.unlockAt+' 点能量）起开放。'}),vs.stock.length?h('div',{class:'grid'},vs.stock.map(function(s){return h('div',{class:'item '+tcl(s.t)+(s.left?'':' lock')},[rimEl(s.t),img('item_'+s.id),h('div',{class:'n'},[h('span',{class:'tag '+tcl(s.t),text:KINDN[s.k]||s.k}),s.name]),h('div',{class:'muted',text:s.desc}),h('div',{class:'row sb',style:'margin-top:6px'},[h('span',{class:'num grow',text:'◆ '+s.price+' · 余 '+s.left}),h('button',{class:'sm pri',disabled:!s.left||m.ls<s.price,onclick:async function(){var x=await rpc('vshop.buy',{idx:s.idx});if(x&&x.ok)renderMarket(Object.assign({},d,x.data))}},'买')])])})):h('div',{class:'muted',text:vs.lv?'今日货已售罄。':'在坊市点「能量供奉」即可开始累计。'})])}
