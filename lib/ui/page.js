...
#wd .tag.vipb{font-size:10px;padding:0 5px;background:rgba(0,0,0,.25)}#wd .vt{border-collapse:collapse;font-size:11px;width:100%}#wd .vt th,#wd .vt td{padding:3px 4px;border-bottom:1px solid var(--line);text-align:center;white-space:nowrap}#wd .vt td:first-child,#wd .vt th:first-child{text-align:left;white-space:normal}#wd .vt .cur{background:rgba(255,255,255,.07);font-weight:bold}#wd .vbig{font-size:13px;font-weight:bold;margin:6px 0;color:#e2b84a}#wd .seal{position:absolute;right:6px;bottom:6px}#wd .name.vip4{text-shadow:0 0 8px rgba(255,154,60,.7)}
/* ---- reward reveal */
...
function vipCard(v){if(!v)return null;var pct=v.need?Math.min(100,Math.round(v.en/v.need*100)):100;var NM=['','白银','黄金','钻石','王者'];var w=v.now||{};
var head=h('tr',{},[h('th',{text:'权益'})].concat([1,2,3,4].map(function(l){return h('th',{class:l===v.lv?'cur':'',style:'color:'+VIPC[l],text:NM[l]+' '+v.all[l].en})})));
var rows=(v.table||[]).map(function(r){return h('tr',{},[h('td',{class:'muted',text:r.n})].concat(r.v.map(function(x,i){var l=i+1;return h('td',{class:(l===v.lv?'cur':l<v.lv?'':'muted'),text:x})})))});
return h('div',{style:'margin-top:12px;border-top:1px solid var(--line);padding-top:8px'},[h('div',{class:'row'},[h('b',{text:'会员 · '+v.name}),vipBadge(v.lv),h('span',{class:'muted',text:'累计供奉 '+v.en+' 点'+(v.next?'，再 '+(v.need-v.en)+' 点升 '+v.next:'，已至顶')})]),h('div',{class:'bar xp'},[h('i',{style:'width:'+pct+'%'})]),v.lv?h('div',{class:'vbig',text:'修炼 ×'+w.rate+' · 离线 +'+w.off+'h · 掉落 ×'+w.drop+' · 每日游历 '+w.exp+' 次 · 供奉 '+w.en+' 点/日'}):h('div',{class:'muted',style:'margin-top:4px',text:'供奉 5 点能量即白银；等级越高，修炼、掉落、离线上限、每日游历与供奉额度全线拉开。'}),h('div',{style:'overflow-x:auto;margin-top:6px'},[h('table',{class:'vt'},[head].concat(rows))]),h('div',{class:'muted',text:'等级按累计供奉的能量升，跨转世永久有效。'})])}
