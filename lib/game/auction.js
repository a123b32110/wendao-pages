...
import { byPrefix, setShared, delShared, sharedRoomFor } from "./shared.js";
...
const slimItem = (it) => ({ k: it.k, id: it.id, name: it.name, n: it.n, t: it.t, q: it.q, rn: (it.rn ?? []).length });

export function auctionsView(c, shared, now) {
  const all = byPrefix(shared, "auction:").map((e) => e.value).filter((a) => a && a.aid);
  const mine = [];
  const open = [];
  const ended = [];
  for (const a of all) {
...
export function bid(c, shared, now, aid, amt, effects) {
  ...
  const a = shared.get(`auction:${aid}`);
...
export function claimAuctions(c, shared, now, effects) {
  ...
  c.aucDone = c.aucDone ?? {};
  c.escrow = c.escrow ?? {};
  const all = byPrefix(shared, "auction:").map((e) => e.value).filter((a) => a && a.aid && now >= a.end);
  for (const a of all) {
  ...
  for (const aid of Object.keys(c.aucDone)) if (!shared.has(`auction:${aid}`)) delete c.aucDone[aid];
  ...
  for (const aid of Object.keys(c.escrow)) if (!shared.has(`auction:${aid}`) && now > (c.escrowEnd?.[aid] ?? 0) + PRUNE_DAYS * DAY) { delete c.escrow[aid]; if (c.escrowEnd) delete c.escrowEnd[aid]; lines.push("一笔逾期未领的托管灵石归于天道。"); }
  return lines;
}
