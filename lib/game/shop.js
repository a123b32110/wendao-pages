...
import { dayEventOfDay } from "./events2.js";

export const SHOP_SLOTS = 10;
export const SHOP_REFRESH_DAILY = 3;
// 白银及以上每日多一次补货；钻石及以上坊市九五折（与商人折扣相乘）
export const shopReLimit = (c) => SHOP_REFRESH_DAILY + ((c.vip | 0) >= 1 ? 1 : 0);
...
export function shopStock(c, day) {
  ...
  // 典籍（v51）只上黄/玄阶的、权重低：坊市是保底来源，地/天阶还得靠妖兽与拍卖
  ...
  if (seeds.length) { const s1 = rng.pick(seeds); add(s1); const rest = seeds.filter((x) => x !== s1); add(rng.pick(rest.length ? rest : seeds)); } // 两格不能撞同一种
  let guard = 0;
